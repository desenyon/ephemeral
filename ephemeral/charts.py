"""Chart generation for Ephemeral using Plotly."""
...
def create_candlestick_chart(
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            close=data["Close"],
            name=symbol.upper(),
            increasing_line_color=EPHEMERAL_THEME["positive"],
            decreasing_line_color=EPHEMERAL_THEME["negative"],
        ),
        row=1, col=1
    )
    ...
    if show_volume and "Volume" in data.columns:
        colors = [EPHEMERAL_THEME["positive"] if data["Close"].iloc[i] >= data["Open"].iloc[i]
                  else EPHEMERAL_THEME["negative"] for i in range(len(data))]
...
def create_line_chart(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=data["Close"],
            name=symbol.upper(),
            line=dict(color=EPHEMERAL_THEME["accent"], width=2),
            fill="tozeroy",
    ...
    if show_volume and "Volume" in data.columns:
        fig.add_trace(
            go.Bar(
                ...
                y=data["Volume"],
                name="Volume",
                marker_color=EPHEMERAL_THEME["neutral"],
                opacity=0.5,
            ),
            row=2, col=1
        )
...
def create_technical_chart(
    ...
    for indicator in indicators:
        if indicator.lower() == "rsi":
            ...
            fig.add_trace(
                go.Scatter(x=data.index, y=rsi, name="RSI", line=dict(color=EPHEMERAL_THEME["accent"])),
                row=row, col=1
            )
            fig.add_hline(y=70, line_dash="dash", line_color=EPHEMERAL_THEME["negative"], row=row, col=1)  # type: ignore
            fig.add_hline(y=30, line_dash="dash", line_color=EPHEMERAL_THEME["positive"], row=row, col=1)  # type: ignore
            fig.update_yaxes(range=[0, 100], row=row, col=1)
            row += 1

        elif indicator.lower() == "macd":
            macd, signal, hist = _calculate_macd(data["Close"])
            colors = [EPHEMERAL_THEME["positive"] if h >= 0 else EPHEMERAL_THEME["negative"] for h in hist]
            ...
            fig.add_trace(
                go.Scatter(x=data.index, y=macd, name="MACD", line=dict(color=EPHEMERAL_THEME["accent"])),
                row=row, col=1
            )
            fig.add_trace(
...
def create_performance_chart(
    ...
    fig.add_hline(
        y=equity_curve[0],
        line_dash="dash",
        line_color=EPHEMERAL_THEME["neutral"],
...
def create_sector_chart(sector_data: dict) -> str:
    ...
    colors = [EPHEMERAL_THEME["positive"] if v >= 0 else EPHEMERAL_THEME["negative"] for v in values]
...
def _apply_layout(fig: go.Figure, title: str, has_volume: bool):
    """Apply Ephemeral theme to chart with publication-grade styling."""
    fig.update_layout(
        title=dict(
            text=f"<b>{title}</b>",
            font=dict(size=20, color=EPHEMERAL_THEME["text_color"], family="SF Pro Display, -apple-system, sans-serif"),
            x=0.5,
            xanchor="center",
        ),
        paper_bgcolor=EPHEMERAL_THEME["paper_color"],
        plot_bgcolor=EPHEMERAL_THEME["bg_color"],
        font=dict(color=EPHEMERAL_THEME["text_color"], family="SF Pro Display, -apple-system, sans-serif", size=12),
        xaxis=dict(
            gridcolor=EPHEMERAL_THEME["grid_color"],
            showgrid=True,
            gridwidth=1,
            zeroline=False,
            showline=True,
            linewidth=1,
            linecolor=EPHEMERAL_THEME["grid_color"],
        ...
        legend=dict(
            ...
            font=dict(color=EPHEMERAL_THEME["text_color"], size=11),
            orientation="h",
            yanchor="bottom",
            y=1.02,
            xanchor="right",
        ...
        hovermode="x unified",
        hoverlabel=dict(
            bgcolor=EPHEMERAL_THEME["bg_color"],
            font_size=12,
            font_family="SF Mono, Menlo, monospace",
            bordercolor=EPHEMERAL_THEME["accent"],
        ),
    )
...
def _save_chart(fig: go.Figure, name: str) -> str:
    ...
    charts_dir = os.path.expanduser("~/.ephemeral/charts")
