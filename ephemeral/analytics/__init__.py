"""Advanced analytics module for Ephemeral."""
...
class RegimeDetector:
    ...
    def correlation_regime(
        ...
        returns_matrix.rolling(window).corr()
...
class FactorAnalyzer:
    ...
    def calculate_factor_exposures(
        ...
        design_matrix = aligned.drop("asset", axis=1).to_numpy(dtype=float)
        design_matrix = np.column_stack([np.ones(len(design_matrix)), design_matrix])  # Add intercept

        # OLS regression
        try:
            coeffs, residuals, rank, s = np.linalg.lstsq(design_matrix, y, rcond=None)
        except Exception:
            return {}
