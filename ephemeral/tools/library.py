"""Financial data tools for Ephemeral."""
...
def get_economic_indicators(indicator: str = "GDP") -> dict:
    ...
    if not api_key:
        return {"error": "Alpha Vantage API key not configured. Set ALPHA_VANTAGE_API_KEY in ~/.ephemeral/config.env"}
...
def search_financial_news(query: str, num_results: int = 5) -> dict:
    ...
    if not api_key:
        return {"error": "Exa API key not configured. Set EXA_API_KEY in ~/.ephemeral/config.env"}
...
def _yfinance_news_fallback(symbol: str, limit: int) -> dict:
    """Headlines from Yahoo Finance via yfinance (no API key)."""
    try:
        ticker = yf.Ticker(symbol.upper())
        raw = ticker.news or []
    except Exception as e:
    ...
    for item in raw[:limit]:
        ...
        summary = (c.get("summary") or c.get("description") or "")[:800]
    ...
    return {"symbol": symbol.upper(), "articles": articles, "source_used": "yfinance"}


def fetch_news_digest(symbol: str = "", query: str = "", limit: int = 10) -> dict:
    """Unified news: Polygon (ticker) → Alpha Vantage → Exa search → Yahoo/yfinance.

    Configure ``POLYGON_API_KEY``, ``ALPHA_VANTAGE_API_KEY``, and/or ``EXA_API_KEY``
    in ``~/.ephemeral/config.env`` for the best coverage. A ticker alone still returns
    Yahoo headlines without keys.
    """
    ...
    if symbol:
        pg = polygon_get_ticker_news(symbol, limit=limit)
        if isinstance(pg, dict) and not pg.get("error") and pg.get("articles"):
            return {
                "symbol": symbol,
                "query": query or None,
                "articles": pg["articles"],
                "source_used": "polygon.io",
                "hint": "Cross-check with fundamentals; headlines can lag price.",
            }
        trail.append(f"polygon:{pg.get('error', 'empty') if isinstance(pg, dict) else pg}")

    av: dict = {}
    if symbol:
        av = get_market_news(tickers=symbol)
    elif query:
        av = get_market_news(topics=query)
    if isinstance(av, dict) and not av.get("error") and av.get("articles"):
        return {
    ...
    if av:
        trail.append(f"alphavantage:{av.get('error', 'empty')}")

    search_q = query or (f"{symbol} stock news catalyst earnings" if symbol else "")
    if search_q:
        ex = search_financial_news(search_q, num_results=min(limit, 10))
        if isinstance(ex, dict) and not ex.get("error") and ex.get("results"):
            articles = []
            for r in ex["results"]:
                articles.append(
                    {
        ...
        if ex:
            trail.append(f"exa:{ex.get('error', 'empty')}")

    if symbol:
        yf_news = _yfinance_news_fallback(symbol, limit)
        if yf_news.get("articles"):
            return {**yf_news, "query": query or None, "trail": trail}

    return {
        "error": "No news retrieved. For tickers, Yahoo data is used when other APIs are unset or rate-limited.",
        "trail": trail,
        "setup_hint": "Add POLYGON_API_KEY or ALPHA_VANTAGE_API_KEY or EXA_API_KEY in ~/.ephemeral/config.env",
    }
...
def generate_comparison_chart(symbols: list, period: str = "1y", normalize: bool = True) -> dict:
    ...
    try:
        from ephemeral.charts import create_comparison_chart
...
TOOLS = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "fetch_news_digest",
            "description": "Preferred unified news feed for a ticker and/or theme. Tries Polygon, Alpha Vantage, Exa, then Yahoo. Use for 'why is X moving' and catalyst summaries.",
            "parameters": {
                "type": "object",
                "properties": {
                    "symbol": {"type": "string", "description": "Primary stock ticker (e.g. NVDA)"},
                    "query": {"type": "string", "description": "Optional theme or search phrase (e.g. semiconductor export rules)"},
                    "limit": {"type": "integer", "description": "Max headlines (1-25)", "default": 10}
                },
                "required": []
            }
        }
...
TOOL_FUNCTIONS = {
    ...
    "fetch_news_digest": fetch_news_digest,
    # Exa Search
