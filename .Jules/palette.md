...
## 2024-05-18 - Hide Irrelevant Keyboard Hints in Multi-Pane CLI
**Learning:** In multi-pane terminal UI environments where single-character shortcuts are heavily utilized, global or omnipresent input hints (like "Enter to run") cause visual confusion when the relevant action pane (like the input pane) is not currently focused or active.
**Action:** Conditionally scope and render keyboard action hints only when their corresponding active pane is focused.
