...
## 2024-06-12 - Hide Input Action Hints When Pane Unfocused
**Learning:** Users can be confused by static action hints like "Enter to run" when the input pane is not actively focused, leading to unexpected behavior if they attempt to press Enter.
**Action:** Conditionally hide interactive action hints when they are not applicable, such as when the terminal is actively busy or the input pane is not focused, to prevent user confusion and visual noise.
