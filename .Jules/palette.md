...
## 2026-07-07 - Focus State Alignment
**Learning:** Visual focus styling (border and text colors) must strictly map to the corresponding logical `focusPane` state to establish clear hierarchy and avoid confusion. Cross-pane logic (like LeftRail checking if RightRail is active) breaks standard CLI application expectations.
**Action:** Applied strictly scoped conditional checking (`focusPane === 'paneName'`) to all pane boundaries and headers.
