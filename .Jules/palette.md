...
## 2026-05-31 - Conditionally hide interactive action hints
**Learning:** Conditionally hide interactive action hints (e.g., 'Enter to run') when they are not applicable, such as when the terminal is actively busy or the input pane is not focused, to prevent user confusion and visual noise.
**Action:** Hide hints using conditional rendering based on current state parameters like `busy` and `focusPane`.
