...
## 2026-05-23 - Hide interactive action hints when inactive
**Learning:** Conditionally hide interactive action hints (e.g., 'Enter to run') when they are not applicable, such as when the terminal is actively busy or the input pane is not focused. This reduces visual noise and aligns UI feedback with the actual state of the application, thereby preventing user confusion.
**Action:** Applied to the terminal UI by hiding 'Enter to run' when `focusPane !== 'input'`.
