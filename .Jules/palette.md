...
## 2024-05-25 - Conditionally Hide Interactive Action Hints
**Learning:** Hiding interactive action hints (e.g., "Enter to run") when their pane is not actively focused reduces visual noise and prevents user confusion since the keyboard action would not be captured anyway.
**Action:** Always conditionally render keyboard shortcuts based on the `focusPane` state to maintain a clear visual hierarchy and avoid misleading the user.
