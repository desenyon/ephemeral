...
## 2024-05-15 - Visual Hierarchy for Interactive Pane States
**Learning:** In a multi-pane terminal interface, keeping headers vividly colored when the pane is inactive creates visual confusion and distracts from the actually focused pane. Similarly, showing action prompts like "Enter to run" when the input pane isn't focused clutters the UI with currently invalid actions.
**Action:** Apply focus-aware dimming by changing inactive pane headers to `gray`, and conditionally render interactive prompts (e.g. "Enter to run") so they only appear when the user is focused on the input pane.
