...
## 2024-06-26 - Hide Inapplicable Action Hints
**Learning:** Showing action hints (like "Enter to run") when a component is not actively focused or actionable creates visual noise and can confuse users navigating via keyboard.
**Action:** Conditionally hide these hints using logical state (e.g., `focusPane === 'input'`) rather than just relying on system status (like `!busy`).
