...
## 2024-05-21 - Conditional Action Hints
**Learning:** Terminal UI UX Pattern: Conditionally hide interactive action hints (e.g., 'Enter to run') when they are not applicable, such as when the terminal is actively busy or the input pane is not focused, to prevent user confusion and visual noise.
**Action:** Added `focusPane === 'input'` to the `{!busy && ( ... )}` check for rendering the "Enter to run" prompt so it is hidden when the input is not focused.
