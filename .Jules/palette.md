...
## 2024-05-29 - Contextual Action Hints in Terminal UI
**Learning:** Displaying interactive action hints like "Enter to run" when the input pane is not actively focused creates visual noise and misleads users into thinking their current input context is ready to execute an action. This can cause frustration when they press Enter and nothing happens because they are navigating a different pane.
**Action:** Conditionally hide interactive action hints (e.g., 'Enter to run') when they are not applicable, such as when the terminal is actively busy or the input pane is not focused, to prevent user confusion and reduce visual clutter.
