...
## 2026-06-22 - Contextual UI Hints & Focus Outlines
**Learning:** In a multi-pane CLI layout, focus states must be exclusive and visually explicit to prevent confusion. Similarly, interactive hints like 'Enter to run' should only be visible when their associated action is contextually active.
**Action:** Ensure boolean logic for focus outlines explicitly maps 1:1 to pane states without cross-pane bleeds, and conditionally hide interactive action hints when the user isn't actively in the text input.
