...
## 2024-06-05 - Contextual Output Hints & Focus Styling
**Learning:** Terminal interfaces often overwhelm users when inactive panes continue to feature bright accent colors and static action hints (like "Enter to run") persist even when the action is no longer possible (e.g., when the input pane isn't focused).
**Action:** Exclusively bind interactive hints to relevant focus states (`focusPane === 'input'`) and aggressively dim headers/selected items in inactive panes to maintain clear visual hierarchy.
